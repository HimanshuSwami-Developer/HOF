...
import { cProject, Client, webProject, otherProject } from "../constants";
...
const ClientsCard = ({
  ...
  return (
    <motion.div whileInView={{ opacity: 1 , transform : 'none'}} variants={fadeIn("up", "spring", index * 0.5, 0.75)}>
      ...
      >
        <div  onClick={() => window.open(source_code_link, "_blank")} className='Box1 relative w-full h-[320px]'>
          <img
            src={image}
            alt='project_image'
    ...
    <>
      ...
      <div className='box mt-20 flex flex-wrap justify-center'>
        {data.map((project, index) => (
          <div>
            <ClientsCard key={`project-${index}`} index={index} {...project} />
          </div>
        ))}
      </div>
